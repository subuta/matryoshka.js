...
export const seekFile = (fileName, predicate = alwaysTrue, chunkSize = 256, encoding = 'utf8', offset = 0) => new Promise((resolve, reject) => {
  ...
  fs.open(fileName, 'r', function (err, fd) {
    ...
    const finalize = (err, buffer) => {
      const pos = _.max([bytesRead - chunkSize, 0])
...
// seek read file and early close file if predicate returns false.
export const readFileByPragma = (fileName, encoding = 'utf8', offset = 0) => new Promise((resolve) => {
  // check data has a START-END pragma
  const checkPragma = (data) => {
    const hasStartPragma = data.match(START_PRAGMA)
    const hasEndPragma = data.match(END_PRAGMA)
  ...
  seekFile(fileName, checkPragma, 256, encoding, offset).then(({data, pos}) => {
    ...
    data = _.transform(data.split(os.EOL), (chunk, line) => {
      ...
      if (line.match(START_PRAGMA)) {
        isRead = true
      } else if (isRead && line.match(END_PRAGMA)) {
        isRead = false
        isDone = true
        chunk.push(line)
      }
...
export const updateFileByPragma = async (fileName, data, encoding = 'utf8') => {
  ...
  await new Promise((resolve, reject) => {
    ...
    // const readStream = fs.createReadStream(fileName, {encoding, flags: 'r'})
    ...
    dataStream
      ...
      .pipe(es.through(function (line) {
        ...
        if (line.match(START_PRAGMA)) {
          ...
          this.pause()
          readFileByPragma(fileName, encoding, offset).then(({data, pos}) => {
            offset = pos
            this.emit('data', data)
            this.resume()
          })
