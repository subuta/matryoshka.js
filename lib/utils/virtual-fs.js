...
export default function createVfs (opts = {}) {
  ...
  const perform = () => {
    ...
    const promises = _.map(pending, (action) => {
      let promise = Promise.resolve(true)
      if (action.type === WRITE_FILE) {
        // skip duplicated changes
        if (_.some(cache, (c) => c.hash === getFileHash(action.payload))) return Promise.resolve(true)

        promise = dryRun ? Promise.resolve(): fs.writeFile(action.payload.fileName, action.payload.data)
    ...
    // remove unused file.
    const deleteFilePromises = _.map(_.difference(oldCache, cache), ({fileName}) => {
      return dryRun ? Promise.resolve(): fs.remove(fileName)
    })

    return Promise.map([...promises, ...deleteFilePromises], (promise) => promise, {concurrency: 1})
  }
